...
class BusinessEntityResourceTest {
    ...
    void testUpdateBusinessEntity() {
        ...
        String newAdrress = given().contentType(ContentType.JSON).body(body).when().put("/business-entities").then().statusCode(201).extract().body().jsonPath()
                .get("address");
    ...
    void testAddBranchToBusinessEntity() {
        ...
        given().when().get(String.format("/business-entities/%d", newBEId)).then().statusCode(200).body("name", equalTo("business entity 14"), "branches.size()", equalTo(2));
